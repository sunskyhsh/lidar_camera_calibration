...
class CameraLiDARCalibrator:
    ...
    '''
    def extract_points_2D(self, img_msg, now, proc_results, rectify=False):
    ...
    '''
    def extract_points_3D(self, pointcloud_msg, now, proc_results):
    ...
    def project_point_cloud(self, pointcloud_msg, img_msg, image_pub):
        ...
        inrange = np.where((np.abs(points3D[:, 0]) < 5) &
                           (points3D[:, 1] > 2) & (points3D[:,1] < 16) & (points3D[:,2] < 4) & (points3D[:,2] > 0.1))
    ...
    def camera_lidar_callback(self, image_msg, pointcloud_msg, image_pub=None):
        ...
        # Calibration mode
            ...
            img_proc = multiprocessing.Process(target=self.extract_points_2D, args=[image_msg, now, self.proc_results])
            ...
            pcl_proc = multiprocessing.Process(target=self.extract_points_3D, args=[pointcloud_msg, now, self.proc_results])
